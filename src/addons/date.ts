...
export const DateFormatter = function (datePattern, dateMin, dateMax) {
    ...
    if (that.dateMin.length === 2) that.dateMin.unshift(0);
    ...
    if (that.dateMax.length === 2) that.dateMax.unshift(0);
    that.initBlocks();
};
...
DateFormatter.prototype = {
    ...
    getRangeFixedDate: function (date) {
        const that = this;
        const datePattern = that.datePattern,
            dateMin = that.dateMin || [],
            dateMax = that.dateMax || [];
    ...
    getFixedDate: function (day, month, year) {
        ...
        if ((month < 7 && month % 2 === 0) || (month > 8 && month % 2 === 1)) {
            day = Math.min(
                day,
                month === 2 ? (that.isLeapYear(year) ? 29 : 28) : 30,
            );
        }
